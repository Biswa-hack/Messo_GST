...
def process_zip_and_combine_data(zip_file_uploader, combo_template_file):
    ...
    try:
        ...
        RAW_FORMULAS = [
            ...
            '=IF(J{0}=$X$22,F{0}*E{0}/100/2,0)',
            # M: (Old L formula) =IF(J1=$X$22,0,F1*E1%)
            '=IF(J{0}=$X$22,0,F{0}*E{0}/100)',
            # N: (Old M formula) =K1+L1+M1+F1
            '=K{0}+L{0}+M{0}+F{0}',
            # O: (Old N formula) =(M1+L1+K1)/F1
            '=(M{0}+L{0}+K{0})/F{0}',
        ]
        ...
        if len(df_merged) > 0:
            # We iterate over the number of rows of merged data
            for r_offset in range(len(df_merged)):
                row_idx = formula_target_start_row + r_offset
            ...
            st.info(f"Hardcoded formulas (J:O) applied to {len(df_merged)} rows.")
